...
// 亮眼
public class BrightEyeFilter3333 extends BaseFrameFilter {
    public static final String TAG = BrightEyeFilter3333.class.getSimpleName();
...
//            0.739348f*(1080/720), 0.758621f*(2137/1280),
    ...
    public BrightEyeFilter3333(Context context) {
...
//        mBlurNextFilter.onReady(width,height);
    }
...
//        return mBlurTexture;
    ...
    protected int mProgramId02;
    protected int vPosition02;
    protected int vCoord02;
    protected int vMatrix02;
    protected int vTexture02;
    int width02;
    int height02;

    protected FloatBuffer mVertexBuffer02;//顶点坐标数据缓冲区
    protected FloatBuffer mTextureBuffer02;//纹理坐标数据缓冲区

    private int drawBlur(int textureID) {
        ...
        mVertexBuffer02 = BufferHelper.getFloatBuffer(VERTEX);
...
//                1.0f, 0.0f,
//        };
        mTextureBuffer02 = BufferHelper.getFloatBuffer(TEXTURE);
        String vertexSource = TextResourceReader.readTextFileFromResource(context, R.raw.base_vertex);
        String fragmentSource = TextResourceReader.readTextFileFromResource(context,
                R.raw.beauty_fragment);
        ...
        mProgramId02 = ShaderHelper.linkProgram(vertexShaderId, fragmentShaderId);
        // TODO 得到程序了，着色器可以释放了
        glDeleteShader(vertexShaderId);
        glDeleteShader(fragmentShaderId);
        //        ShaderHelper.validateProgram(mProgramId);
        vPosition = glGetAttribLocation(mProgramId02, "vPosition");
        vCoord02 = glGetAttribLocation(mProgramId02, "vCoord");
        vMatrix02 = glGetUniformLocation(mProgramId02, "vMatrix");
        vTexture02 = glGetUniformLocation(mProgramId02, "vTexture");

        width02 = glGetUniformLocation(mProgramId02, "width"); // 关联最新改变的宽
        height02 = glGetUniformLocation(mProgramId02, "height"); // 关联最新改变的高
        ...
        // 2：使用着色器程序
        glUseProgram(mProgramId02);

        // 渲染 传值
        // 1：顶点数据
        mVertexBuffer02.position(0);
        glVertexAttribPointer(vPosition02, 2, GL_FLOAT, false, 0, mVertexBuffer02); // 传值
        glEnableVertexAttribArray(vPosition02); // 传值后激活

        // 2：纹理坐标
        mTextureBuffer.position(0);
        glVertexAttribPointer(vCoord02, 2, GL_FLOAT, false, 0, mTextureBuffer02); // 传值
        glEnableVertexAttribArray(vCoord02); // 传值后激活

        // TODO 给纹理宽高 赋值  只有这个点和以前不同，其他的 全部都是模板代码 都是之前的
        glUniform1i(width02, mWidth);
        glUniform1i(height02, mHeight);

        // 片元 vTexture
        glActiveTexture(GL_TEXTURE0); // 激活图层
        glBindTexture(GL_TEXTURE_2D, textureID); // 绑定
        glUniform1i(vTexture02, 0); // 传递参数
