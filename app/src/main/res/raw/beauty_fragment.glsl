...
void main() {
    ...
    */
    ...
    highPassColor.r = clamp(2.0*highPassColor.r*highPassColor.r * 24.0, 0.0, 1.0);// 【注意：需要反复去调】
...
//    gl_FragColor = highPassBlur;// TODO 2：高反差，先不磨皮，先运行看看效果了【只能看到五官轮廓黑边边了】
    ...
    // TODO 3：磨皮
    // 蓝色分量  min glsl着色器语言的内置函数：取两个参数的最小值
    float blue = min(currentColor.b, blur.b);// 取两个参数的最小值
    float value = clamp((blue - 0.2) * 5.0, 0.0, 1.0);// 【注意：需要反复去调】 经验值  最开始做美颜功能前辈
    ...
    float currentIntensity = (1.0 - maxChannelColor / (maxChannelColor + 0.2)) * value * intensity;// 【注意：需要反复去调】 经验值  最开始做美颜功能前辈
    ...
    vec4 nColor = vec4(rgb_value, 1.0);// gl_FragColor: rgb不支持 yuv   TODO 3：磨皮，看看效果
